...
The certificate of \cref{sec:conformal} holds empirically. \Cref{tab:conformal} reports coverage over 10 seeds and four datasets. The \emph{gate}---coverage under the worst-case \AEGIS attack at magnitude $\varepsilon$---holds at the nominal $1{-}\alpha{=}0.90$ at $\varepsilon{=}0.01$ on every dataset and is conservative ($0.94$--$1.00$) at $\varepsilon{=}0.05$. Sets run ${\sim}1.0$--$3.5$ labels, non-vacuous throughout and tightest where the deterministic radius is thin, at zero Monte-Carlo samples. Exchangeability on a single transductive graph is a stated condition---the empirical gate is the evidence---and $S_c$ is formed densely at $N{=}200$, a matrix-free conformal pass being future work.
