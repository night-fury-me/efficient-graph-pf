...
\begin{table}[t]
\centering
\caption{One-query audit vs.\ iterative attackers: equilibrium damage at $\varepsilon{=}0.10$ (IGNN, 50-node subgraph, 10 seeds). \AEGIS's single analytic SVD query matches or beats a $50$-step PGD on equilibrium shift, and per query delivers $74$--$156\times$ its damage. Cls-PGD: classification-loss PGD; Shift-PGD: IFT-gradient PGD (solver validation, not an independent baseline).}
...
\begin{table}[t]
\centering
\caption{The $\sigma_1(S_c)$ penalty trades clean accuracy for certified robustness (Cora, $c{=}0.9$ IGNN, 10 seeds). Raising $\lambda$ cuts the dominant sensitivity $\sigma_1$, shrinks worst-case attack damage, and enlarges the certified fraction; cert.\ frac peaks at $\lambda{=}3{\times}10^{-3}$ before margin collapse. The operating point $\lambda{=}3{\times}10^{-4}$ buys a $10\times$ drop in $\sigma_1$ for ${\sim}4$ accuracy points.}
\label{tab:defense}
\footnotesize
\setlength{\tabcolsep}{4pt}
