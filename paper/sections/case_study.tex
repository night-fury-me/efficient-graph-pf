...
As a proof-of-concept cross-domain demonstration — not an operator-grade screening tool — we apply $S_c$ to AC power flow. The conceptual correspondence is a structural isomorphism: the IFT-resolvent $(I{-}J_z)^{-1}$ at the GNN's equilibrium is the linearised analogue of the post-contingency steady-state operator, so $v_{ij}$ (continuous edge sensitivity) tracks N-1 contingency severity (discrete line removal) the way PTDF/LODF~\cite{wood2014power} linearise line trips for DC-PF screening. Recent GNN-based PF work~\cite{donon2019graph,bolz2019power,kim2025physicsinformed,nakiganda2023graph} optimises a fidelity objective; we take the vulnerability-analysis view.

\textbf{Setup.} ContractiveGCN-PF is an IGNN-class operator $F(Z,A){=}\phi(\Ahat ZW^\top + X_\mathrm{proj})$ with spectral-norm-constrained $W$, trained on five IEEE test cases~\cite{ieee2024testcases} with a 2-head readout ($\Delta|V|, \Delta\theta$). Training data are 2{,}000 load samples per case ($70$--$130\%$ of nominal) from PandaPower's~\cite{thurner2018pandapower} AC Newton-Raphson solver; inputs are binary adjacency and 5 bus features (slack/PV indicators, $P$, $Q$, $|V|$). For case14--118 we use full-graph dense analysis; for case300 a 200-node BFS subgraph. LODF is benchmarked off-spec on $\ell_2$ voltage-angle truth (its native metric is line-flow change), so the ordering reflects the GNN's training target.
