...
\label{eq:ift}
\end{equation}
Equation~\eqref{eq:ift} is the lever \AEGIS pulls: it gives the closed-form first-order change in $\zstar$ under any parameter $p$, with the resolvent $(I-J_z)^{-1}$ amplifying a perturbation by at most $1/(1-\kappa)$ via its convergent Neumann series. The pseudospectral index $\eta{=}\norm{(I-J_z)^{-1}}_2(1-\rho) \geq 1$~\cite{trefethen2005spectra} measures the gap to the spectral-radius bound ($\eta{=}1$ for normal $J_z$). The IFT has served hyperparameter optimization~\cite{lorraine2020optimizing} and implicit differentiation~\cite{gould2021deep}, but not adversarial structural analysis.
