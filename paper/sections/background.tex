...
\label{eq:ift}
\end{equation}
Equation~\eqref{eq:ift} is the lever \AEGIS pulls: it gives the closed-form first-order change in $\zstar$ under any parameter $p$. Here the resolvent $(I-J_z)^{-1}$ acts as a gain that amplifies the raw perturbation by at most $1/(1-\kappa)$ through its convergent Neumann series, so the closer the model drifts to losing contractivity ($\kappa\!\to\!1$), the more violently its equilibrium can move. The phase transition of \cref{sec:theory} makes this intuition precise. One refinement matters there: the pseudospectral index $\eta{=}\norm{(I-J_z)^{-1}}_2(1-\rho) \geq 1$~\cite{trefethen2005spectra} measures how far the true gain sits above its spectral-radius estimate, equalling 1 exactly when $J_z$ is normal.
