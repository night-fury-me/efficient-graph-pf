...
\label{eq:ift}
\end{equation}
Equation~\eqref{eq:ift} is the lever \AEGIS pulls: the closed-form first-order change in $\zstar$ under any parameter $p$. The resolvent $(I-J_z)^{-1}$ acts as a gain bounded by $1/(1-\kappa)$, so as the model nears the loss of contractivity ($\kappa\!\to\!1$) its equilibrium moves ever more violently; \cref{sec:theory} makes this phase transition precise, refined by the pseudospectral index $\eta{=}\norm{(I-J_z)^{-1}}_2(1-\rho){\geq}1$~\cite{trefethen2005spectra} ($\eta{=}1$ for normal $J_z$).
