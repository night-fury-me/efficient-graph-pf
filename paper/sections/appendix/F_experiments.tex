...
\Cref{fig:positioning} positions these methods on a capability radar. No single method covers every
axis: structural attacks win raw damage at large budgets, certifiers win the sound per-node
certificate, and \AEGIS is the only one that reads an attack direction, per-edge attribution, and a
first-order per-node certificate off a single closed-form, label-free $S_c$ query.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_positioning_radar.pdf}
\caption{Method-positioning radar (axis levels qualitative; $0$ at the centre, $1$ at the outer ring).
Structural attacks~\citep{zugner2018adversarial,zugner2019adversarial,geisler2021robustness} surface an
attack direction and per-edge ranks and dominate raw damage at large budgets, but give no per-node
certificate; certifiers~\citep{schuchardt2023localized,li2025agnncert} give sound per-node radii but
name no vulnerability-driving edges; \AEGIS reads attack direction, per-edge attribution, and a
first-order per-node certificate off a single closed-form $S_c$ query, label-free and at far lower
cost. The specialists peak on their own axis; \AEGIS is the unified option covering all six.}
