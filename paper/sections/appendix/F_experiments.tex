...
\Cref{fig:positioning} places these methods on a capability matrix: prior attacks surface attack
direction or per-edge rank on surrogate losses, prior certifiers surface per-node radii but no
direction, and \AEGIS surfaces all four from a single closed-form $S_c$ under realistic (symmetric,
edge-only) perturbations.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_positioning.pdf}
\caption{Capability matrix of \AEGIS against prior structural-vulnerability methods across four axes
(attack direction, per-edge rank, per-node radius, realistic edge-only perturbation). Legend keys
resolve to the reference list: [1] Nettack~\citep{zugner2018adversarial}; [2]
Mettack~\citep{zugner2019adversarial}; [3] GR-BCD~\citep{geisler2021robustness}; [4] localized
smoothing~\citep{schuchardt2023localized}; [5] AGNNCert~\citep{li2025agnncert}. Attacks ($\bullet$)
surface attack-direction or per-edge information on surrogate losses; certifiers ($\bullet$) surface
per-node radii but no direction or rank; \AEGIS ($\bullet$) surfaces all four from one closed-form
computation of $S_c$.}
