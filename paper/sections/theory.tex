...
\begin{proof}
\textbf{Part (a).} The Jacobian of $F$ with respect to $\mathrm{vec}(Z)$ takes the form $J_z = \mathrm{diag}(\phi') \cdot (\Ahat \otimes W)$, where $\phi' \in \{0, 1\}^{Nd}$ for ReLU (A1). The operator norm satisfies $\kappa = \norm{J_z}_2 \leq \norm{\Ahat}_2 \cdot \norm{W}_2 \cdot \sup|\phi'| = \norm{\Ahat}_2 \cdot \norm{W}_2$. Applying the IFT to $G(z,A) = z - F(z,A) = 0$ at $(\zstar, A)$:
\begin{equation}
