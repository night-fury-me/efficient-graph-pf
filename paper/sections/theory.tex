...
These sensitivity radii are deterministic (not probabilistic), unlike randomized smoothing~\cite{bojchevski2020efficient} which holds with probability $1-\alpha$. They are first-order quantities: locally tight for small $\varepsilon$ but increasingly conservative as perturbation magnitude grows. Without bounding the second-order remainder $O(\norm{\delta A}_F^2)$, they are local sensitivity measures rather than global robustness certificates. \cref{sec:adaptive} verifies empirically that the radii hold (0\% breach rate at $\varepsilon = 0.01$, $< 1\%$ at $\varepsilon = 0.10$).
