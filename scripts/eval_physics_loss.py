...
# When invoked as `python scripts/eval_physics_loss.py`, Python puts `scripts/` at the
...
log = logging.getLogger("eval_physics_loss")
...
def _load_state_dict(ckpt_path: str) -> dict[str, torch.Tensor]:
    ...
    if not isinstance(payload, dict):
        raise TypeError(
            f"Checkpoint must be a state_dict or {{'state_dict': ...}} mapping, got {type(payload).__name__}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Evaluate physics loss for a checkpoint on the configured dataset. "
            "Supports full, LoRA, head-only, and hybrid variants."
        )
    )
    ...
    cfg.add_argument(
        ...
        dest="base_config",
        default=None,
        help="Base YAML config (e.g., configs/default.yaml). Use with --scenario for overlay-style configs.",
    )
    ...
    p.add_argument(
        ...
        default=None,
        help="Optional scenario overlay YAML (e.g., configs/scenarios/lora_ft_hv.yaml).",
    )
    ...
    p.add_argument(
        ...
        nargs="+",
        default=None,
        help="Optional parquet path(s) to override data.parquet_paths in config.",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Override seed used for dataset split (default: config run.seed).",
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "If omitted, will use config.peft.lora_r; if that mismatches the checkpoint, the script auto-detects r."
        ),
    )
    p.add_argument(
        "--lora-alpha",
        ...
        help=(
            ...
            "Note: alpha is not stored in the checkpoint state_dict in this repo."
        ),
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Evaluation batch size. If omitted, uses min(config.train.batch_size, 32).",
    )

    p.add_argument(
        "--out-dir",
        default=None,
        help="Output directory for metrics artifacts. Default: results/physics_eval/<timestamp>_<ckpt_stem>",
    )

    p.add_argument(
        "--no-force-pinn",
        action="store_true",
        help="Use flags.pinn from config instead of forcing physics loss.",
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.no_force_pinn:
        pinn = bool(get(merged_raw, ("flags", "pinn"), True))
    else:
        # Force physics loss evaluation by default.
        pinn = True
    ...
    mlflow_experiment = str(get(merged_raw, ("mlflow", "experiment"), "PhysicsEval"))
    ...
    out_dir = Path(args.out_dir) if args.out_dir else Path("results") / "physics_eval" / f"{timestamp}_{ckpt_stem}"
    ...
    manifest = {
        ...
        "pinn": {
            "forced": bool(not args.no_force_pinn),
            "enabled": bool(pinn),
        },
        "peft": {
    ...
    log.info("Building dataloaders (%d parquet files)", len(parquet_paths))
    ...
    metrics = {
        "test/phys_loss": float(m.loss),
        "n_train": float(splits.n_train),
        "n_val": float(splits.n_val),
        "n_test": float(splits.n_test),
    }
    ...
    tags.update(
        {
            "eval_type": "physics_loss",
            "ckpt_type": "lora" if is_lora_ckpt else "full",
            "ckpt": str(ckpt_path),
            "seed": str(seed),
            "pinn_forced": str(bool(not args.no_force_pinn)).lower(),
        }
    )

    run_name = f"physics_{Path(ckpt_path).stem}"
    with mlflow_run(
        ...
        if mlf is not None:
            ...
            try:
                log_run_artifacts(
                    mlflow=mlf,
                    run_dir=out_dir,
                    artifact_path=os.path.join(mlflow_artifact_path, "physics_eval"),
                )
            except Exception:
                pass

    log.info("Done. Metrics written to %s", out_dir)
    log.info("TEST | phys_loss=%.6g", metrics["test/phys_loss"])
