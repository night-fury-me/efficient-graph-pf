...
DATASET_LABELS = ["Cora", "Citeseer", "Pubmed", "WikiCS", "Amazon Photo"]
...
def main():
    ...
    vmax = max(0.9, np.nanmax(np.abs(M)) + 0.05)
    ...
    # Annotate cells + mark anti-correlated and OOM cells with DISTINCT
    # patterns so the eye can separate "missing data" from "negative tau".
    # - anti-correlated (tau < -0.05): diagonal hatch + value in white bbox
    # - OOM:                            dotted grid + italic "OOM" tag
    for i in range(M.shape[0]):
        for j in range(M.shape[1]):
            ...
            if v < -0.05:
                # Anti-correlated: diagonal hatch overlay AND a white
                # rounded-corner bbox behind the value label so the digits
                # stay legible against the hatching.
                ax.add_patch(mpatches.Rectangle(
                ...
                ax.text(j, i, f"{v:+.2f}", ha="center", va="center",
                        fontsize=9, color="#222222", family="serif",
                        bbox=dict(facecolor="white", edgecolor="none",
                                  boxstyle="round,pad=0.14", alpha=0.92),
                        zorder=3)
            else:
                ax.text(j, i, f"{v:+.2f}", ha="center", va="center",
                        fontsize=9, color=color, family="serif",
                        zorder=3)
    ...
    cbar.set_ticklabels([r"$-0.5$", r"$0$", r"$+0.5$"])
    ...
    fig.text(
        0.5, 0.015,
        r"$^\dagger$ GAT-2: 2-layer attention; OOM on Pubmed and WikiCS.",
