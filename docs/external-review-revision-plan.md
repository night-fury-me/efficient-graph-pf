...
**Current state (updated 2026-05-30):** revision in progress — `paper/aegis.pdf` compiles cleanly at **10 pages** (latexmk exit 0; 0 errors / 0 undefined refs / 0 overfull). Per-item completion status in the **Progress Log** below. The R2 experiment round (R2_01–R2_10, `docs/r2_experiments_full_report.md`) is run and integrated (commit `2d4e7c2`), closing **X1/X2/X3**; **E3** and **E5** are now done. Remaining work: the unrun evidence experiments **X9, X4, X5** (and optional **X7**), plus code release (**R1**). The overview diagram (C1) is corrected, re-exported, and re-embedded.
...
| **E2** | ✅ reframe · ⏳ X7 opt. | Shift-PGD flagged as solver-validation; "matches 50-step PGD" not "recovers only 72–92%"; SVD optimal for the first-order hidden-state objective. Independent baseline X7 optional. | `abstract.tex`, `experiments.tex` |
...
| **R1** | ⏳ pending | Release diagnostic-only code path (+1 clarifying sentence). | repo, `framework.tex` |
...
| **X9** | ⏳ pending | *Earn the phase transition*: stress a model so a real eigenvalue→+1 and exhibit Ω(1/(ε_crit−ε)) blow-up. `fig:phase_transition` currently shows the trained-safe side (ρ≤0.42, 2–4× margin) against the theoretical worst-case 1/(1−κ) curve; the empirical stress-blow-up is not yet run. | `experiments.tex` |
| **X4** | ⏳ pending | Fixed- vs recomputed-normalization deletion: Kendall τ between rankings (validates T4). Not in the R2 set. | `experiments.tex` |
| **X5** | ⏳ pending | Min-over-classes radius recompute (refresh paper numbers for the corrected T3 form). | `experiments.tex` |
| **X7** *(opt.)* | ⏳ pending | Independent black-box/transfer attack vs the SVD direction (elevates E2 from defended to proven). | `experiments.tex` |
...
**Remaining to ship:** **X9, X4, X5** (unrun, need GPU), optional **X7**, and **R1** (diagnostic-only code release). Everything else in the table is done.
